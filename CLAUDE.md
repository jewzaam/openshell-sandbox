...
- **Context generation detects PRs from git, not manifest.** `generate_repo_context()`
  runs `gh pr view <branch>` on the host — if the local branch name is `pr-N`
  (from `clone_repo_host`), it passes the number; otherwise the branch name.
  Manifest ref is updated to match, not the other way around. Switching branches
  locally and re-uploading regenerates context automatically.
